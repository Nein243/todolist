<?php
if (!isset($_SESSION['login'])) {
    header('location:registration.php');
    exit;
}
?>
    <div class="navigation">
        <h2 class="nav-header">Menu</h2>
        <ul class="navigation-section" id="categories">
            <li class="navigation-row">
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20" height="20" viewBox="0 0 16 16">
                    <path d="M 7.5 1 C 3.917969 1 1 3.917969 1 7.5 C 1 11.082031 3.917969 14 7.5 14 C 11.082031 14 14 11.082031 14 7.5 C 14 3.917969 11.082031 1 7.5 1 Z M 7.5 2 C 10.542969 2 13 4.457031 13 7.5 C 13 10.542969 10.542969 13 7.5 13 C 4.457031 13 2 10.542969 2 7.5 C 2 4.457031 4.457031 2 7.5 2 Z M 7 4 L 7 7 L 4 7 L 4 8 L 7 8 L 7 11 L 8 11 L 8 8 L 11 8 L 11 7 L 8 7 L 8 4 Z"></path>
                </svg>
                <a class="task-add" href="add_task.php">Add a task</a>
            </li>
            <div class="section-header">Tasks</div>
            <li class="navigation-row">
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="16" height="16" viewBox="0 0 16 16">
                    <path d="M 2.5 2 C 1.671875 2 1 2.671875 1 3.5 L 1 12.5 C 1 13.074219 1.328125 13.566406 1.804688 13.816406 L 3.800781 8.019531 C 4.003906 7.410156 4.574219 7 5.21875 7 L 15 7 L 15 5.5 C 15 4.671875 14.328125 4 13.5 4 L 6.796875 4 L 6.144531 2.789063 C 5.882813 2.300781 5.375 2 4.824219 2 Z M 5.21875 8 C 5.003906 8 4.8125 8.136719 4.746094 8.34375 L 2.796875 14 L 13.5 14 C 14.328125 14 15 13.328125 15 12.5 L 15 8 Z"></path>
                </svg>
                <a href="index.php">All tasks</a>
            </li>
            <li class="navigation-row">
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="16" height="16" viewBox="0 0 16 16">
                    <path d="M 2 2 L 2 12 C 2 13.097656 2.902344 14 4 14 L 12 14 C 13.097656 14 14 13.097656 14 12 L 14 5 L 11 5 L 11 2 Z M 3 3 L 10 3 L 10 12 C 10 12.367188 10.109375 12.703125 10.28125 13 L 4 13 C 3.441406 13 3 12.558594 3 12 Z M 4 5 L 4 6 L 9 6 L 9 5 Z M 11 6 L 13 6 L 13 12 C 13 12.558594 12.558594 13 12 13 C 11.441406 13 11 12.558594 11 12 Z M 4 7 L 4 8 L 6 8 L 6 7 Z M 7 7 L 7 8 L 9 8 L 9 7 Z M 4 9 L 4 10 L 6 10 L 6 9 Z M 7 9 L 7 10 L 9 10 L 9 9 Z M 4 11 L 4 12 L 6 12 L 6 11 Z M 7 11 L 7 12 L 9 12 L 9 11 Z"></path>
                </svg>
                <a href="done_list.php">Done</a>
            </li>
            <li class="navigation-row">
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="16" height="16" viewBox="0 0 16 16">
                    <path d="M 6.496094 1 C 5.675781 1 5 1.675781 5 2.496094 L 5 3 L 2 3 L 2 4 L 3 4 L 3 12.5 C 3 13.324219 3.675781 14 4.5 14 L 10.5 14 C 11.324219 14 12 13.324219 12 12.5 L 12 4 L 13 4 L 13 3 L 10 3 L 10 2.496094 C 10 1.675781 9.324219 1 8.503906 1 Z M 6.496094 2 L 8.503906 2 C 8.785156 2 9 2.214844 9 2.496094 L 9 3 L 6 3 L 6 2.496094 C 6 2.214844 6.214844 2 6.496094 2 Z M 4 4 L 11 4 L 11 12.5 C 11 12.78125 10.78125 13 10.5 13 L 4.5 13 C 4.21875 13 4 12.78125 4 12.5 Z M 5 5 L 5 12 L 6 12 L 6 5 Z M 7 5 L 7 12 L 8 12 L 8 5 Z M 9 5 L 9 12 L 10 12 L 10 5 Z"></path>
                </svg>
                <a href="deleted_list.php">Deleted</a>
            </li>
            <li class="navigation-row">
                <svg width="16px" height="16px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M21 10H3M21 12.5V8.8C21 7.11984 21 6.27976 20.673 5.63803C20.3854 5.07354 19.9265 4.6146 19.362 4.32698C18.7202 4 17.8802 4 16.2 4H7.8C6.11984 4 5.27976 4 4.63803 4.32698C4.07354 4.6146 3.6146 5.07354 3.32698 5.63803C3 6.27976 3 7.11984 3 8.8V17.2C3 18.8802 3 19.7202 3.32698 20.362C3.6146 20.9265 4.07354 21.3854 4.63803 21.673C5.27976 22 6.11984 22 7.8 22H12M16 2V6M8 2V6M14.5 19L16.5 21L21 16.5"
                          stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
                <a href="">Calendar</a>
            </li>
            <div class="section-header">Categories</div>
            <?php require_once 'category.php'; ?>
            <li class="navigation-row category-link">
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20" height="20" viewBox="0 0 16 16">
                    <path d="M 7.5 1 C 3.917969 1 1 3.917969 1 7.5 C 1 11.082031 3.917969 14 7.5 14 C 11.082031 14 14 11.082031 14 7.5 C 14 3.917969 11.082031 1 7.5 1 Z M 7.5 2 C 10.542969 2 13 4.457031 13 7.5 C 13 10.542969 10.542969 13 7.5 13 C 4.457031 13 2 10.542969 2 7.5 C 2 4.457031 4.457031 2 7.5 2 Z M 7 4 L 7 7 L 4 7 L 4 8 L 7 8 L 7 11 L 8 11 L 8 8 L 11 8 L 11 7 L 8 7 L 8 4 Z"></path>
                </svg>
                <a href="#">Add a category</a>
            </li>
            <li>
                <?php require_once 'add_category.php'; ?>
            </li>
            <div class="section-header">Category groups</div>
            <li class="navigation-row category-group-link">
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20" height="20" viewBox="0 0 16 16">
                    <path d="M 7.5 1 C 3.917969 1 1 3.917969 1 7.5 C 1 11.082031 3.917969 14 7.5 14 C 11.082031 14 14 11.082031 14 7.5 C 14 3.917969 11.082031 1 7.5 1 Z M 7.5 2 C 10.542969 2 13 4.457031 13 7.5 C 13 10.542969 10.542969 13 7.5 13 C 4.457031 13 2 10.542969 2 7.5 C 2 4.457031 4.457031 2 7.5 2 Z M 7 4 L 7 7 L 4 7 L 4 8 L 7 8 L 7 11 L 8 11 L 8 8 L 11 8 L 11 7 L 8 7 L 8 4 Z"></path>
                </svg>
                <a href="category-group-page.php">Add a category group</a>
            </li>

        </ul>
    </div>
    <main>
<?php
require_once 'user-block.php';
?>