...
$sharedTeams = getSharedTeamsByUserId($_SESSION['id'])
?>
<div class="nav-categories">
<?php foreach ($sharedTeams as $sharedTeam): ?>
    <li class="navigation-row category-row" data-category-id="<?= $sharedTeam['id'] ?>">
        <a href="team-view.php?id=<?= $sharedTeam['id']; ?>"><?= $sharedTeam['title']; ?></a>
        <div class="category-setting">
            <button title="Edit" data-team-id="<?= $sharedTeam['id'] ?>" data-team-title="<?= $sharedTeam['title'] ?>"
            ...
            <form action="" class="delete-team" method="post">
                <input type="hidden" value="<?= $sharedTeam['id'] ?>" name="id">
