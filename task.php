...
$pdo = getPDO();
$query = $pdo->prepare('SELECT tasks.id, tasks.title, tasks.text, categories.color FROM tasks INNER JOIN categories ON tasks.category_id=categories.id WHERE tasks.status = "new" AND tasks.user_id = :user_id');
$query->execute([
        'user_id' => $_SESSION['id']
]);
...
foreach ($tasks as $task):?>
    <div class="task-item" style="background-color:<?= $task['color'] ?>">
        ...
        <div class="task-setting">
            <a href="handler/done-task_handler.php?id=<?= $task['id']?>" title="Mark as done" class="task-setting_object" id="done-task">
            ...
            <a href="handler/delete-task_handler.php?id=<?= $task['id']?>" title="Mark as deleted" class="task-setting_object">
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24" height="24" viewBox="0 0 16 16">
